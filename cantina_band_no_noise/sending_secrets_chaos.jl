...
tspan = (0,60.0) # length of the message is 5 seconds 
...
### Receiver ###
# Get secret message 
secret_message = convert_message_to_samples("CantinaBand60Secret.wav")

# Decrpyt message 
decrypted_message = decrypt_secret_message(u0, p, tspan, secret_message)

# Get the sampling rate and number of samples 
sample, sampling_rate = wavread("CantinaBand60Secret.wav")
sample = vec(sample)
num_of_samples = length(sample)
